...
class NeuralNetwork():
	...
	def __init__(self, image_height, image_width, n_actions, num_stacked_frames = 4):
		...
		# DEFINE VARIABLES
		self.W1 = tf.Variable(tf.truncated_normal([8,8,self.num_frames, 16],mean=0.0,stddev=0.1))
		self.b1 = tf.Variable(tf.constant(0.1, shape=[16]))

		self.W2 = tf.Variable(tf.truncated_normal([8,8,16,16],mean=0.0,stddev=0.1))
		...
		self.W4 = tf.Variable(tf.truncated_normal([20*20*16, 128],mean=0.0, stddev=0.1))
		self.b4 = tf.Variable(tf.constant(0.1, shape=[128]))

		self.W5 = tf.Variable(tf.truncated_normal([128,n_actions],mean=0.0, stddev=0.1))
		...
		# SAVER
		self.saver = tf.train.Saver({"W1":self.W1,"b1":self.b1,"W2":self.W2,"b2":self.b2,"W4":self.W4,"b4":self.b4,"W5":self.W5,"b5":self.b5})
		...
		sess = self.sess
		if restore_model:
			self.saver.restore(sess,"drive/ML_Projects/Colab Notebooks/")
		else:
	...
	def copy_from(self, other):
		...
		self.sess.run(self.W5.assign(other.sess.run(other.W5)))
		self.sess.run(self.b5.assign(other.sess.run(other.b5)))
...
def get_return(model_1,model_2, s_prime,a,r, target_model):
	...
	## Double DQN modifies double q-learning slightly so that one architecture is 
	## used entirely to evaluate the choice of action, while the other 
	## architecture is used entirely to greedily choose the action
...
def main(N=100):
	...
	while True:
		if record_bool not in ["Y","n"]:
			...
			break
	if record_bool=="Y":
		env = gym.wrappers.Monitor(env, "videos_breakout_003",force=True,
				video_callable=lambda episode_id: episode_id%15==0)
	else:
		pass
	...
	for i in range(N):
		...
		if i%10 == 0 and i!=0:
			tot_run_avg/= 50
			print("episode = ",i, " avg over 10 = ",tot_run_avg)
			running_average.append(tot_run_avg)
			tot_run_avg = 0
		if i%25 == 0 and SAVE==True:
			model.save_model("dDQN_model")
			model_2.save_model()

	plt.plot(running_average)
	plt.xlabel("No. games (x10)")
	plt.ylabel("10-Game Time Average")
	plt.show()
