...
class NeuralNetwork():
	...
	def __init__(self, n_observations, n_actions):
		...
		selected_action_values = y_pred * self.A
...
def get_return(model_1,model_2, s_prime,a,r, target_model):
	...
	## model_1 is the main model,
	## model_2 is the target network. 
...
def main(N=100):
	...
	model = NeuralNetwork(D,K)	# MAIN NETWORK
	model_2 = NeuralNetwork(D,K) # TARGET NETWORK
	...
	for i in range(N):
		...
		if i%5 == 0 and i!=0:
			tot_run_avg/= 5
			print("episode = ",i, " avg over 5 = ",tot_run_avg)
			running_average.append(tot_run_avg)
			tot_run_avg = 0

	plt.plot(running_average)
	plt.xlabel("No. games (x100)")
	plt.ylabel("5-Game Time Average")
	plt.show()
